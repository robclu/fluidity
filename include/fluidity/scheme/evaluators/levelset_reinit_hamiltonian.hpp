//==--- ../../evaluators/levelset_reinit_hamiltonian.hpp --- -*- C++ -*- ---==//
//            
//                                Fluidity
// 
...
//==------------------------------------------------------------------------==//
//
/// \file  levelset_reinit_hamiltonian.hpp
/// \brief This file defines an implementation of the Evaluatable interface
///        which evaluates data based on the levelset reinitialization
///        hamiltonian.
//
//==------------------------------------------------------------------------==//

#ifndef FLUIDITY_SCHEME_EVALUATORS_LEVELSET_REINIT_HAMILTONIAN_HPP
#define FLUIDITY_SCHEME_EVALUATORS_LEVELSET_REINIT_HAMILTONIAN_HPP

namespace fluid     {
namespace scheme    {
namespace evaluator {

/// The LevelsetReinitHamiltonian class implements an evaluatable class which
/// can evaluate the levelset reinitialization hamiltonian using a Godunov
/// upwind spatial discretization and the stencil provided by the template type.
/// This allows any type of stencil for the derivatives to be used to compose
/// numerous levelset reinitialization Hamiltonians with different accuracy.
/// \tparam Stencil The spatial stencil to use with the Godunov upwind method.
template <typename Stencil>
class LevelsetReinitHamiltonian 
  : public Evaluatable<LevelsetReinitHamiltonian<Scheme>> {
 private:
 ...
 public:
  ...
  evaluate_impl(It&& it, T dh, VIt&& v_it, Args&&... args) const
  {
    static_assert(is_multidim_iter_v<It>,
      "Iterator for LevelsetReinitHamiltonian must "
      "be a multi-dimensional iterator!");
  ...
  /// \tparam    Args The types of the additional arguments for the functor.
  template <typename    It  ,
            typename    T   ,
            typename    F   ,
            typename... Args, nonmultiit_enable_t<F> = 0>
  fluidity_host_device auto
  evaluate_impl(It&& it, T dh, F&& f, Args&&... args) const
  {
    static_assert(is_multidim_iter_v<It>,
      "Iterator for LevelsetReinitHamiltonian must "
      "be a multi-dimensional iterator!");
...
#endif // FLUIDITY_SCHEME_EVALUATORS_LEVELSET_REINIT_HAMILTONIAN_HPP
