...
int main(int argc, char** argv) {
  ...
  fill(input.multi_iterator(), [&] fluidity_host_device (auto& cell) {
    constexpr auto center_x = static_cast<real_t>(size_x) / 3.0;
    constexpr auto center_y = static_cast<real_t>(size_y) / 3.0;
    constexpr auto radius   = static_cast<real_t>(size_x) / 15.0;
