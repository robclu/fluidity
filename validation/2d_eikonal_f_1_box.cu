...
#include <fluidity/geometry/box.hpp>
...
static constexpr auto size_x = int{1000};
static constexpr auto size_y = int{1000};
...
int main(int argc, char** argv) {
  ...
  fill(input.multi_iterator(), [&] fluidity_host_device (auto& cell)
  {
    ...
    constexpr auto length_x = static_cast<real_t>(size_y) / 6.0;
    constexpr auto length_y = static_cast<real_t>(size_y) / 5.0;
    ...
    *cell = Box<real_t>{
      vec3_t(length_x, length_y, 0.0), pos_t{center_x, center_y, 0.0}
    }.distance(p);
  });
